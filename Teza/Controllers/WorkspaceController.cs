...
namespace Teza.Controllers
{
     ...
     public class WorkspaceController : ControllerBase
     {
          ...
          public WorkspaceController(RepositoryDbContext context, IAuthService authService, IMailingService mailingService, ICronService cronService)
          {
          ...
          public async Task<ActionResult<object>> ConfirmEmail([FromRoute] Guid workspaceId, [FromRoute] string email, [FromRoute] string confirmationToken)
          {
               try
               {
                    ...
                    if (confirmation != null && email == confirmation.Email)
                    {
                         ...
                         {
                              EntityName = email,
                              EntityType = EntityType.collaborator,
                              UserName = email,
          ...
          public async Task<ActionResult<object>> GetQueryById([FromRoute] Guid queryId)
          {
               try
               {
                    ...
                    var qryToReturn = new GetQueryForCronService
                    {
                         Id = queryId,
                         RawSql = query.RawSql
                    };
          ...
          public async Task<ActionResult<object>> SetUpCron([FromBody] CronParamsModel cronParams)
          {
               try
               {
                    ...
                    if (string.IsNullOrEmpty(cronId))
                    {
                         ...
                         {
                              success = false,
                              error = "CronId has not been retrieved from CronService."
                         };
                    }
                    ...
                    _unitOfWork.CronRepository.Create(cronJob);
                    await _unitOfWork.SaveChangesAsync();

                    return new SuccessModel
                    {
                         message = "Cron setup successful.",
                         success = true
                    };
          ...
          [HttpPost("stopcron/{cronId}")]
          [ServiceFilter(typeof(AuthorizationAttribute))]
          public async Task<ActionResult<object>> StopCron([FromRoute] Guid cronId)
          {
               try
               {
                    var cron = await _unitOfWork.CronRepository.GetByCondition(x => x.Id == cronId).FirstOrDefaultAsync();
                    ...
                    if (cron == null)
                    {
                         ...
                         {
                              success = false,
                              error = "There's no cron with such an ID."
                         };
                    }
                    ...
                    if (!cronIsDeleted)
                    {
                         ...
                         {
                              success = false,
                              error = "Something went wrong with the removal of this cron job."
                         };
                    }

                    _unitOfWork.CronRepository.Delete(cron);
                    await _unitOfWork.SaveChangesAsync();
